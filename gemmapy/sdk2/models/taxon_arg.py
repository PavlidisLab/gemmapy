...
TAXONARG_ONE_OF_SCHEMAS = ["int", "str"]

class TaxonArg(BaseModel):
    """
    TaxonArg
    """
    # data type: int
    ...
    if TYPE_CHECKING:
        ...
        actual_instance: Any
    one_of_schemas: List[str] = Field(TAXONARG_ONE_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in TaxonArg with oneOf schemas: int, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in TaxonArg with oneOf schemas: int, str. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> TaxonArg:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> TaxonArg:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into TaxonArg with oneOf schemas: int, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into TaxonArg with oneOf schemas: int, str. Details: " + ", ".join(error_messages))
        else:
            return instance
