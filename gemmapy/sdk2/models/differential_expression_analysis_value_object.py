...
class DifferentialExpressionAnalysisValueObject(BaseModel):
    """
    DifferentialExpressionAnalysisValueObject
    """
    id: Optional[StrictInt] = None
    factor_values_used: Optional[Dict[str, conlist(FactorValueValueObject)]] = Field(None, alias="factorValuesUsed")
    result_sets: Optional[conlist(DiffExResultSetSummaryValueObject)] = Field(None, alias="resultSets")
    array_designs_used: Optional[conlist(StrictInt)] = Field(None, alias="arrayDesignsUsed")
    bio_assay_set_id: Optional[StrictInt] = Field(None, alias="bioAssaySetId")
    source_experiment: Optional[StrictInt] = Field(None, alias="sourceExperiment")
    subset_factor: Optional[ExperimentalFactorValueObject] = Field(None, alias="subsetFactor")
    subset_factor_value: Optional[FactorValueValueObject] = Field(None, alias="subsetFactorValue")
    factor_values_used_by_experimental_factor_id: Optional[Dict[str, conlist(FactorValueValueObject)]] = Field(None, alias="factorValuesUsedByExperimentalFactorId")
    is_subset: Optional[StrictBool] = Field(None, alias="isSubset")
    __properties = ["id", "factorValuesUsed", "resultSets", "arrayDesignsUsed", "bioAssaySetId", "sourceExperiment", "subsetFactor", "subsetFactorValue", "factorValuesUsedByExperimentalFactorId", "isSubset"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DifferentialExpressionAnalysisValueObject:
        """Create an instance of DifferentialExpressionAnalysisValueObject from a JSON string"""
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of each value in factor_values_used (dict of array)
        _field_dict_of_array = {}
        if self.factor_values_used:
            for _key in self.factor_values_used:
                if self.factor_values_used[_key]:
                    _field_dict_of_array[_key] = [
                        _item.to_dict() for _item in self.factor_values_used[_key]
                    ]
            _dict['factorValuesUsed'] = _field_dict_of_array
        # override the default output from pydantic by calling `to_dict()` of each item in result_sets (list)
        _items = []
        if self.result_sets:
            for _item in self.result_sets:
                if _item:
                    _items.append(_item.to_dict())
            _dict['resultSets'] = _items
        # override the default output from pydantic by calling `to_dict()` of subset_factor
        if self.subset_factor:
            _dict['subsetFactor'] = self.subset_factor.to_dict()
        # override the default output from pydantic by calling `to_dict()` of subset_factor_value
        if self.subset_factor_value:
            _dict['subsetFactorValue'] = self.subset_factor_value.to_dict()
        # override the default output from pydantic by calling `to_dict()` of each value in factor_values_used_by_experimental_factor_id (dict of array)
        _field_dict_of_array = {}
        if self.factor_values_used_by_experimental_factor_id:
            for _key in self.factor_values_used_by_experimental_factor_id:
                if self.factor_values_used_by_experimental_factor_id[_key]:
                    _field_dict_of_array[_key] = [
                        _item.to_dict() for _item in self.factor_values_used_by_experimental_factor_id[_key]
                    ]
            _dict['factorValuesUsedByExperimentalFactorId'] = _field_dict_of_array
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> DifferentialExpressionAnalysisValueObject:
        """Create an instance of DifferentialExpressionAnalysisValueObject from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DifferentialExpressionAnalysisValueObject.parse_obj(obj)

        _obj = DifferentialExpressionAnalysisValueObject.parse_obj({
            "id": obj.get("id"),
            "factor_values_used": dict(
                (_k,
                        [FactorValueValueObject.from_dict(_item) for _item in _v]
                        if _v is not None
                        else None
                )
                for _k, _v in obj.get("factorValuesUsed").items()
            ),
            "result_sets": [DiffExResultSetSummaryValueObject.from_dict(_item) for _item in obj.get("resultSets")] if obj.get("resultSets") is not None else None,
            "array_designs_used": obj.get("arrayDesignsUsed"),
            "bio_assay_set_id": obj.get("bioAssaySetId"),
            "source_experiment": obj.get("sourceExperiment"),
            "subset_factor": ExperimentalFactorValueObject.from_dict(obj.get("subsetFactor")) if obj.get("subsetFactor") is not None else None,
            "subset_factor_value": FactorValueValueObject.from_dict(obj.get("subsetFactorValue")) if obj.get("subsetFactorValue") is not None else None,
            "factor_values_used_by_experimental_factor_id": dict(
                (_k,
                        [FactorValueValueObject.from_dict(_item) for _item in _v]
                        if _v is not None
                        else None
                )
                for _k, _v in obj.get("factorValuesUsedByExperimentalFactorId").items()
            ),
            "is_subset": obj.get("isSubset")
        })
        return _obj
