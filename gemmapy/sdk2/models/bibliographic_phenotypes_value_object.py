...
class BibliographicPhenotypesValueObject(BaseModel):
    """
    BibliographicPhenotypesValueObject
    """
    gene_ncbi: Optional[StrictInt] = Field(None, alias="geneNCBI")
    gene_name: Optional[StrictStr] = Field(None, alias="geneName")
    evidence_id: Optional[StrictInt] = Field(None, alias="evidenceId")
    phenotypes_values: Optional[conlist(CharacteristicValueObject, unique_items=True)] = Field(None, alias="phenotypesValues")
    all_id_of_phenotype: Optional[conlist(StrictInt)] = Field(None, alias="allIdOfPhenotype")
    __properties = ["geneNCBI", "geneName", "evidenceId", "phenotypesValues", "allIdOfPhenotype"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> BibliographicPhenotypesValueObject:
        """Create an instance of BibliographicPhenotypesValueObject from a JSON string"""
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of each item in phenotypes_values (list)
        _items = []
        if self.phenotypes_values:
            for _item in self.phenotypes_values:
                if _item:
                    _items.append(_item.to_dict())
            _dict['phenotypesValues'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> BibliographicPhenotypesValueObject:
        """Create an instance of BibliographicPhenotypesValueObject from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return BibliographicPhenotypesValueObject.parse_obj(obj)

        _obj = BibliographicPhenotypesValueObject.parse_obj({
            "gene_ncbi": obj.get("geneNCBI"),
            "gene_name": obj.get("geneName"),
            "evidence_id": obj.get("evidenceId"),
            "phenotypes_values": [CharacteristicValueObject.from_dict(_item) for _item in obj.get("phenotypesValues")] if obj.get("phenotypesValues") is not None else None,
            "all_id_of_phenotype": obj.get("allIdOfPhenotype")
        })
        return _obj
