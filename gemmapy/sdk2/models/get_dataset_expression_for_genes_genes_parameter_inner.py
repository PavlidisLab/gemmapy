...
GETDATASETEXPRESSIONFORGENESGENESPARAMETERINNER_ONE_OF_SCHEMAS = ["str"]

class GetDatasetExpressionForGenesGenesParameterInner(BaseModel):
    """
    GetDatasetExpressionForGenesGenesParameterInner
    """
    # data type: str
    oneof_schema_1_validator: Optional[StrictStr] = Field(None, description="An Ensembl gene identifier which typically starts with 'ENSG'.")
    # data type: str
    oneof_schema_2_validator: Optional[StrictStr] = Field(None, description="An NCBI gene identifier.")
    # data type: str
    oneof_schema_3_validator: Optional[StrictStr] = Field(None, description="An official gene symbol approved by HGNC.")
    if TYPE_CHECKING:
        actual_instance: Union[str]
    else:
        actual_instance: Any
    one_of_schemas: List[str] = Field(GETDATASETEXPRESSIONFORGENESGENESPARAMETERINNER_ONE_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = GetDatasetExpressionForGenesGenesParameterInner.construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in GetDatasetExpressionForGenesGenesParameterInner with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in GetDatasetExpressionForGenesGenesParameterInner with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> GetDatasetExpressionForGenesGenesParameterInner:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> GetDatasetExpressionForGenesGenesParameterInner:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into GetDatasetExpressionForGenesGenesParameterInner with oneOf schemas: str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into GetDatasetExpressionForGenesGenesParameterInner with oneOf schemas: str. Details: " + ", ".join(error_messages))
        else:
            return instance
