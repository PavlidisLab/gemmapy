...
class ExperimentExpressionLevelsValueObject(BaseModel):
    """
    ExperimentExpressionLevelsValueObject
    """
    dataset_id: Optional[StrictInt] = Field(None, alias="datasetId")
    gene_expression_levels: Optional[conlist(GeneElementExpressionsValueObject)] = Field(None, alias="geneExpressionLevels")
    __properties = ["datasetId", "geneExpressionLevels"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ExperimentExpressionLevelsValueObject:
        """Create an instance of ExperimentExpressionLevelsValueObject from a JSON string"""
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of each item in gene_expression_levels (list)
        _items = []
        if self.gene_expression_levels:
            for _item in self.gene_expression_levels:
                if _item:
                    _items.append(_item.to_dict())
            _dict['geneExpressionLevels'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> ExperimentExpressionLevelsValueObject:
        """Create an instance of ExperimentExpressionLevelsValueObject from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ExperimentExpressionLevelsValueObject.parse_obj(obj)

        _obj = ExperimentExpressionLevelsValueObject.parse_obj({
            "dataset_id": obj.get("datasetId"),
            "gene_expression_levels": [GeneElementExpressionsValueObject.from_dict(_item) for _item in obj.get("geneExpressionLevels")] if obj.get("geneExpressionLevels") is not None else None
        })
        return _obj
