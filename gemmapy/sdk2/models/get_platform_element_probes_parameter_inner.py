...
GETPLATFORMELEMENTPROBESPARAMETERINNER_ONE_OF_SCHEMAS = ["int", "str"]

class GetPlatformElementProbesParameterInner(BaseModel):
    """
    GetPlatformElementProbesParameterInner
    """
    # data type: int
    oneof_schema_1_validator: Optional[StrictInt] = Field(None, description="A composite sequence numerical identifier.")
    # data type: str
    oneof_schema_2_validator: Optional[StrictStr] = Field(None, description="A composite sequence name.")
    if TYPE_CHECKING:
        actual_instance: Union[int, str]
    else:
        actual_instance: Any
    one_of_schemas: List[str] = Field(GETPLATFORMELEMENTPROBESPARAMETERINNER_ONE_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = GetPlatformElementProbesParameterInner.construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in GetPlatformElementProbesParameterInner with oneOf schemas: int, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in GetPlatformElementProbesParameterInner with oneOf schemas: int, str. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> GetPlatformElementProbesParameterInner:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> GetPlatformElementProbesParameterInner:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into GetPlatformElementProbesParameterInner with oneOf schemas: int, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into GetPlatformElementProbesParameterInner with oneOf schemas: int, str. Details: " + ", ".join(error_messages))
        else:
            return instance
