...
class ExternalDatabaseValueObject(BaseModel):
    """
    ...
    external_databases: Optional[conlist(ExternalDatabaseValueObject, unique_items=True)] = Field(None, alias="externalDatabases")
    __properties = ["id", "name", "description", "uri", "releaseVersion", "releaseUrl", "lastUpdated", "externalDatabases"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ExternalDatabaseValueObject:
        """Create an instance of ExternalDatabaseValueObject from a JSON string"""
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of each item in external_databases (list)
        _items = []
        if self.external_databases:
            for _item in self.external_databases:
                if _item:
                    _items.append(_item.to_dict())
            _dict['externalDatabases'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> ExternalDatabaseValueObject:
        """Create an instance of ExternalDatabaseValueObject from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ExternalDatabaseValueObject.parse_obj(obj)

        _obj = ExternalDatabaseValueObject.parse_obj({
            "id": obj.get("id"),
            "name": obj.get("name"),
            ...
            "external_databases": [ExternalDatabaseValueObject.from_dict(_item) for _item in obj.get("externalDatabases")] if obj.get("externalDatabases") is not None else None
        })
        return _obj

ExternalDatabaseValueObject.update_forward_refs()
