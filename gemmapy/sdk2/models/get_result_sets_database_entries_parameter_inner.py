...
GETRESULTSETSDATABASEENTRIESPARAMETERINNER_ONE_OF_SCHEMAS = ["int", "str"]

class GetResultSetsDatabaseEntriesParameterInner(BaseModel):
    """
    GetResultSetsDatabaseEntriesParameterInner
    """
    # data type: int
    oneof_schema_1_validator: Optional[StrictInt] = Field(None, description="A database entry numerical identifier.")
    # data type: str
    oneof_schema_2_validator: Optional[StrictStr] = Field(None, description="A database entry name.")
    if TYPE_CHECKING:
        actual_instance: Union[int, str]
    else:
        actual_instance: Any
    one_of_schemas: List[str] = Field(GETRESULTSETSDATABASEENTRIESPARAMETERINNER_ONE_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = GetResultSetsDatabaseEntriesParameterInner.construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in GetResultSetsDatabaseEntriesParameterInner with oneOf schemas: int, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in GetResultSetsDatabaseEntriesParameterInner with oneOf schemas: int, str. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> GetResultSetsDatabaseEntriesParameterInner:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> GetResultSetsDatabaseEntriesParameterInner:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into GetResultSetsDatabaseEntriesParameterInner with oneOf schemas: int, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into GetResultSetsDatabaseEntriesParameterInner with oneOf schemas: int, str. Details: " + ", ".join(error_messages))
        else:
            return instance
