...
class BioMaterialValueObject(BaseModel):
    """
    ...
    bio_assay_ids: Optional[conlist(StrictInt)] = Field(None, alias="bioAssayIds")
    characteristics: Optional[conlist(CharacteristicValueObject)] = None
    factors: Optional[Dict[str, StrictStr]] = None
    factor_value_objects: Optional[conlist(IdentifiableValueObject)] = Field(None, alias="factorValueObjects")
    factor_values: Optional[conlist(IdentifiableValueObject)] = Field(None, alias="factorValues")
    __properties = ["id", "name", "description", "bioAssayIds", "characteristics", "factors", "factorValueObjects", "factorValues"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> BioMaterialValueObject:
        """Create an instance of BioMaterialValueObject from a JSON string"""
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in factor_value_objects (list)
        _items = []
        if self.factor_value_objects:
            for _item in self.factor_value_objects:
                if _item:
                    _items.append(_item.to_dict())
            _dict['factorValueObjects'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in factor_values (list)
        _items = []
        if self.factor_values:
            for _item in self.factor_values:
                if _item:
                    _items.append(_item.to_dict())
            _dict['factorValues'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> BioMaterialValueObject:
        """Create an instance of BioMaterialValueObject from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return BioMaterialValueObject.parse_obj(obj)

        _obj = BioMaterialValueObject.parse_obj({
            "id": obj.get("id"),
            "name": obj.get("name"),
            "description": obj.get("description"),
            "bio_assay_ids": obj.get("bioAssayIds"),
            "characteristics": [CharacteristicValueObject.from_dict(_item) for _item in obj.get("characteristics")] if obj.get("characteristics") is not None else None,
            "factors": obj.get("factors"),
            "factor_value_objects": [IdentifiableValueObject.from_dict(_item) for _item in obj.get("factorValueObjects")] if obj.get("factorValueObjects") is not None else None,
            "factor_values": [IdentifiableValueObject.from_dict(_item) for _item in obj.get("factorValues")] if obj.get("factorValues") is not None else None
        })
        return _obj
